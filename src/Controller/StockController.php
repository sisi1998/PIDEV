...
class StockController extends AbstractController
{
    ...
    public function new(Request $request, StockRepository $stockRepository): Response
    {
        if ($request->isMethod('POST')) {
            // Récupération de la valeur du champ "quantite"
            $quantite = $request->request->get('quantite');

            // Validation de la valeur de la quantité
            assert($quantite !== null && filter_var($quantite, FILTER_VALIDATE_INT) !== false && $quantite >= 0, "La quantité doit être un entier positif");
